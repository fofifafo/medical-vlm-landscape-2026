...
- `data/medical_vlm_corpus_1217.csv` — the 1,217 medical VLM papers (arXiv id, date, title) with one column per tag (1 = pattern matched title + abstract). Abstracts are not redistributed; fetch them by arXiv id. `data/tag_names.csv` maps each column to the tag name used inside the code and its regex.
- Note: code comments and the tag identifiers inside `medical_taxonomy.py` are in Korean; `data/tag_names.csv` and `Supplementary_Material.pdf` give the English names and patterns.
